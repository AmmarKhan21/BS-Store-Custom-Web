  ...
  const loadDatabaseAssets = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Could not query DB engine status:", err);
      }
...
console.log("First product:", prods);
      ...
      if (Array.isArray(ords)) {
        ...
        console.error("Expected orders to be an array, got:", ords);
        setOrders([]);
      }
      ...
      triggerToast("Network issue: Unable to bind live database backend.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleAddReview = async (productId: string, newReview: any) => {
    try {
      ...
      if (data.success) {
        triggerToast("Review posted successfully! Recalculated ratings on Live SQL database.");
      ...
      console.error("Failed to post review in SQLite DB:", e);
      triggerToast("Error saving review to SQLite.");
    }
  };
  ...
  // --- PERSISTENT SQL MERCHANT ADMIN PORTAL HANDLERS ---
  const handleMerchantAddProduct = async (newProduct: Product) => {
    try {
      ...
      if (response.ok) {
        triggerToast(`"${newProduct.name}" listed successfully in persistent SQL database.`);
        const prodRes = await fetch("/api/products");
        setProducts(await prodRes.json());
      }
    } catch (e) {
      console.error("SQLite insert failure:", e);
    }
  };
  ...
  const handleMerchantUpdateProduct = async (updatedProduct: Product) => {
    try {
      ...
      if (response.ok) {
        triggerToast(`"${updatedProduct.name}" details updated successfully in SQLite database.`);
        const prodRes = await fetch("/api/products");
        const fresh = await prodRes.json();
        setProducts(fresh);

        // Update active shopping cart records with new details
        setCart((prevCart) =>
          prevCart.map((item) => {
            const matching = fresh.find((p: Product) => p.id === item.product.id);
            return matching ? { ...item, product: matching } : item;
          })
        );
      }
    } catch (e) {
      console.error("SQLite update failure:", e);
    }
  };
  ...
  const handleMerchantDeleteProduct = async (productId: string) => {
    try {
      ...
      if (response.ok) {
        triggerToast("Product deleted from SQL relational store.");
        const prodRes = await fetch("/api/products");
        setProducts(await prodRes.json());
      }
    } catch (e) {
      console.error("SQLite delete failure:", e);
    }
  };
  ...
  const handleMerchantUpdateStock = async (productId: string, newStock: number) => {
    try {
      ...
      console.error("SQLite stock update failure:", e);
    }
  };
  ...
  const handleMerchantAddCoupon = async (newCoupon: Coupon) => {
    try {
      ...
      if (response.ok) {
        triggerToast(`Discount Code "${newCoupon.code}" saved in SQLite coupons table.`);
        const coupRes = await fetch("/api/coupons");
        setCoupons(await coupRes.json());
      }
    } catch (e) {
      console.error("SQLite coupon registration failure:", e);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 font-sans selection:bg-indigo-100 flex flex-col justify-between">
      ...
      <div className="bg-slate-900 border-b border-slate-800 text-slate-200 text-xs py-2 px-3 sm:px-6 md:px-8 flex flex-col md:flex-row justify-between items-center gap-2 relative z-40">
        <div className="flex items-center gap-3">
          {isSupabase ? (
            <div className="flex items-center gap-2 bg-indigo-950/80 border border-indigo-800/80 px-2.5 py-1 rounded-full">
              <span className="w-2 h-2 rounded-full bg-indigo-400 animate-pulse" />
              <span className="font-bold text-indigo-300 font-mono text-[10px] uppercase tracking-wider">Supabase Live (Prisma ORM)</span>
            </div>
          ) : (
            <div className="flex items-center gap-2 bg-amber-950/80 border border-amber-800/80 px-2.5 py-1 rounded-full">
              <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" />
              <span className="font-bold text-amber-300 font-mono text-[10px] uppercase tracking-wider">Local JSON Sandbox (Prisma Ready)</span>
            </div>
          )}
          <p className="font-mono text-slate-400 text-[10px] sm:text-xs">
            {isSupabase ? (
              <span className="hidden md:inline text-slate-300">Successfully synced to your Supabase PostgreSQL cloud database!</span>
            ) : (
              <span className="hidden md:inline text-slate-400">Add <code className="bg-slate-800 text-amber-200 px-1 py-0.5 rounded text-[11px]">DATABASE_URL</code> to secrets to connect to your live Supabase!</span>
            )}
          </p>
        </div>
              ...
              <div className="flex-1 space-y-5">
                ...
                {isLoading ? (
                  <div className="flex flex-col items-center justify-center py-24 bg-white border border-slate-200 rounded-2xl">
                    ...
                    <p className="text-xs text-slate-500 font-sans">Connecting to live SQL relational database...</p>
                  </div>
